...
async function loadLinks() {
  try {
    ...
    linkList.innerHTML = links.map((link, index) => `
      <a class="link-row" href="/${encodeURIComponent(link.slug)}" style="animation-delay:${index * 70}ms">
        <span><span class="link-title">${escapeHtml(link.label)}</span><span class="link-path">jtp.fyi/${escapeHtml(link.slug)}</span></span>
